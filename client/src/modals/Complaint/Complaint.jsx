...
function AddComplaint({ setShowModal }) {
    ...
    async function handleSubmit(e) {
        ...
        if (validForm()) {
            const {data} = await axios.post("/user/complaint", {complaintAgainst, description});
            if(!data.err){
                Swal.fire(
                    'Success!',
                    'Your complaint is registered with complaint Id "'+data.complaint.complaintId+'". We will take actions within 2 days.',
                    'success'
                )
                setShowModal(false)
            }
                        ...
                        <Form.Select aria-label="Default select example" value={complaintAgainst} onChange={(e) => setComplaintAgainst(e.target.value)}>
                            <option value="">Select Guide</option>
                            {
                                guides.map((item, index)=>{
                                    return <option key={index} value={item._id}>{item.firstName}</option>
                                })
                            }
